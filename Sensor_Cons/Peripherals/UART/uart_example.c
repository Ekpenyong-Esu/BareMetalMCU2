...
{
    log_debug("Initializing UART with mode: %d", mode);
    ...
    log_debug("Config created with mode: %d", config.mode);
    ...
    if (config.mode == UART_MODE_DMA) {
        ...
        if (status != UART_OK) {
            log_debug("Failed to start DMA reception");
            return status;
        }
        ...
        status = UART_Receive(&uartHandle, uartHandle.rxBuffer, uartHandle.rxSize, 0);
        if (status != UART_OK) {
            log_debug("Failed to start interrupt reception");
            return status;
        }
    }
...
void UART_Example_PreProcess(UART_Handle_t* handle)
{
    if (handle == NULL || handle->rxBuffer == NULL) {
        log_debug("Invalid handle or buffer");
        return;
    }
    ...
    if (handle->config.mode == UART_MODE_DMA) {
        ...
        if (status != UART_OK) {
            log_debug("Blocking receive failed");
            return;
        }
...
{
    if (cmd == NULL) {
        log_debug("NULL command received");
        return UART_ERROR;
    }
    ...
    log_debug("Processing command: %s", cleanCmd);
    ...
    if (strcmp(cleanCmd, CMD_DMA) == 0) {
        ...
        if (status != UART_OK) {
            log_debug("Failed to deinitialize UART");
            return status;
        }
        ...
        if (status != UART_OK) {
            log_debug("Failed to initialize UART in DMA mode");
            return status;
        }
    ...
    if (strcmp(cleanCmd, CMD_INTERRUPT) == 0) {
        ...
        if (status != UART_OK) {
            log_debug("Failed to initialize UART in Interrupt mode");
            return status;
        }
    ...
    if (strcmp(cleanCmd, CMD_BLOCKING) == 0) {
        ...
        if (status != UART_OK) {
            log_debug("Failed to initialize UART in Blocking mode");
            return status;
        }
...
UART_Status_t UART_Example_SendMessage(const char* msg)
{
    if (msg == NULL || uartHandle.huart == NULL) {
        log_debug("UART handle or message is NULL");
        return UART_ERROR;
    }

    uint16_t length = strlen(msg);
    if (length == 0 || length >= TX_BUFFER_SIZE - 1) {
        log_debug("Message length is invalid");
        return UART_ERROR;
    }
    ...
    if (status != UART_OK) {
        log_debug("Send message failed: %d", status);
    }
...
void UART_Example_MainLoop(void)
{
    ...
    if (UART_Example_Init(UART_MODE_INTERRUPT) != UART_OK) {
        log_debug("UART Example initialization failed");
        return;
    }

    log_debug("UART Example main loop started - Mode: %s",
                uartHandle.config.mode == UART_MODE_DMA ? "DMA" :
