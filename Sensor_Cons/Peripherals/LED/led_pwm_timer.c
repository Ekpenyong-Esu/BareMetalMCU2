...
bool LedPwmTimer_Init(LedPwmTimer_t* pwm, const LedPwmTimerConfig_t* config)
{
    ...
    if (TIM_PWM_InitHz(&pwm->htim, config->timer, config->pwmFrequencyHz,
                       LED_PWM_TIMER_STEPS) != HAL_OK) {
        return false;
    }

    if (!LedPwmTimer_PinInit(config)) {
        return false;
    }

    pwm->channel = config->channel;

    return (TIM_PWM_ConfigChannel(&pwm->htim, pwm->channel, 0u) == HAL_OK) &&
           (TIM_PWM_Start(&pwm->htim, pwm->channel) == HAL_OK);
}
