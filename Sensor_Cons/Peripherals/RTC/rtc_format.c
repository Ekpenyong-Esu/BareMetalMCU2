...
#include "stm32f4xx_hal.h" /* RTC_HOURFORMAT12_AM / _PM */
#include <stdio.h>

/* Private variables ---------------------------------------------------------*/

/** Indexed by RTC_WeekDay_Definitions (1..7); slot 0 covers an unset field */
static const char *const rtc_weekdayNames[8] = {"",    "Mon", "Tue", "Wed",
                                                "Thu", "Fri", "Sat", "Sun"};
...
void RTC_FormatTimeString(const RTC_Time_t *sTime, char *buffer, size_t size) {
    ...
    if (sTime->TimeFormat == RTC_HOURFORMAT12_PM || sTime->TimeFormat == RTC_HOURFORMAT12_AM) {
        (void)snprintf(buffer, size, "%02u:%02u:%02u %s", (unsigned)sTime->Hours,
                       (unsigned)sTime->Minutes, (unsigned)sTime->Seconds,
                       (sTime->TimeFormat == RTC_HOURFORMAT12_PM) ? "PM" : "AM");
    }
    else {
        (void)snprintf(buffer, size, "%02u:%02u:%02u", (unsigned)sTime->Hours,
                       (unsigned)sTime->Minutes, (unsigned)sTime->Seconds);
    }
}
...
void RTC_FormatDateString(const RTC_Date_t *sDate, char *buffer, size_t size) {
    ...
    (void)snprintf(buffer, size, "%s %02u/%02u/%04u", rtc_weekdayNames[weekday],
