...
static const uint32_t rtc_daysInMonth[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
uint32_t RTC_DateTimeToTimestamp(const RTC_Date_t* sDate, const RTC_Time_t* sTime)
{
    ...
    if (sDate->Month < 1U || sDate->Month > 12U || sDate->Date < 1U)
    {
        return 0;
    }
    ...
    for (uint32_t y = RTC_EPOCH_BASE_YEAR; y < year; y++)
    {
        days += RTC_IsLeapYear(y) ? 366U : 365U;
    }

    /* Whole months of the current year */
    for (uint32_t m = 1U; m < sDate->Month; m++)
    {
        days += RTC_DaysInMonth(m, year);
    }
    ...
    return (days * RTC_SECONDS_PER_DAY) +
           ((uint32_t)sTime->Hours * RTC_SECONDS_PER_HOUR) +
           ((uint32_t)sTime->Minutes * RTC_SECONDS_PER_MINUTE) +
           sTime->Seconds;
}
