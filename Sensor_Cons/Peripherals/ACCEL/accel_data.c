...
ACCEL_StatusTypeDef ACCEL_ReadRawData(int16_t *xAxis, int16_t *yAxis, int16_t *zAxis)
{
    ...
    ACCEL_StatusTypeDef status = ACCEL_ReadRegisters(ACCEL_REG_OUT_X_MSB, buffer, ACCEL_SAMPLE_BYTES);
    if (status != ACCEL_OK)
    {
        return status;
    }
...
ACCEL_StatusTypeDef ACCEL_ReadData(ACCEL_DataTypeDef *data)
{
    ...
    ACCEL_StatusTypeDef status = ACCEL_ReadRawData(&data->X, &data->Y, &data->Z);
    if (status != ACCEL_OK)
    {
        return status;
    }

    const uint8_t range = ACCEL_GetCachedRange();
