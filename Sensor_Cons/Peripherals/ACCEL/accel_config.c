...
ACCEL_StatusTypeDef ACCEL_SetMode(uint8_t mode)
{
    for (size_t i = 0; i < (sizeof(s_modes) / sizeof(s_modes[0])); i++)
    {
        ...
        {
            return ACCEL_UpdateRegister(ACCEL_REG_CTRL_REG1, s_modes[i].mask, s_modes[i].value);
        }
    }
...
ACCEL_StatusTypeDef ACCEL_SetDataRate(uint8_t odr)
{
    ...
    return ACCEL_UpdateRegister(ACCEL_REG_CTRL_REG1,
                                ACCEL_CTRL_REG1_ODR_MASK,
...
ACCEL_StatusTypeDef ACCEL_GetRange(uint8_t *range)
{
    ...
    ACCEL_StatusTypeDef status = ACCEL_ReadRegister(ACCEL_REG_XYZ_DATA_CFG, &cfg);
    if (status == ACCEL_OK)
    {
...
{
    return ACCEL_WriteRegister(ACCEL_REG_HP_FILTER_CUTOFF,
...
ACCEL_StatusTypeDef ACCEL_ConfigInterrupts(const ACCEL_IntConfigTypeDef *config)
{
    ...
    ACCEL_StatusTypeDef status = ACCEL_WriteRegister(ACCEL_REG_CTRL_REG4, mask);
    if (status != ACCEL_OK)
    {
        return status;
    }

    return ACCEL_WriteRegister(ACCEL_REG_CTRL_REG5, mask);
}
