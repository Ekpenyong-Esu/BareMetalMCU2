...
static HAL_StatusTypeDef IR_DecodeNEC(IR_Handle_t *handle) {
    ...
    address = (uint8_t)(data >> IR_NEC_ADDRESS_SHIFT);
    addressInv = (uint8_t)(data >> IR_NEC_ADDRESS_INV_SHIFT);
    command = (uint8_t)(data >> IR_NEC_COMMAND_SHIFT);
    commandInv = (uint8_t)data;

    if ((address ^ addressInv) != IR_NEC_BYTE_MASK || (command ^ commandInv) != IR_NEC_BYTE_MASK) {
        return HAL_ERROR;
    }
...
static HAL_StatusTypeDef IR_DecodeRC5(IR_Handle_t *handle) {
    ...
    startBits = (uint8_t)((data >> IR_RC5_START_SHIFT) & 0x03U);
    if (startBits != IR_RC5_START_BITS) {
        return HAL_ERROR;
    }

    handle->rxFrame.protocol = IR_PROTOCOL_RC5;
    handle->rxFrame.address = (data >> IR_RC5_ADDRESS_SHIFT) & IR_RC5_MAX_ADDRESS;
