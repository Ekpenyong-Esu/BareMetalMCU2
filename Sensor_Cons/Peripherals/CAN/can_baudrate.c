...
/** @brief One whole bit expressed in permille */
#define CAN_PERMILLE_FULL 1000U
/** @brief Half a permille unit, added before dividing to round to nearest */
#define CAN_PERMILLE_ROUND 500U

/* Private functions ---------------------------------------------------------*/

/**
 * @brief Place the sample point inside a bit of @p quanta time quanta
 *
 * Computes BS1/BS2 so the sample point lands near 87.5% of the bit,
 * clamped to the BS1/BS2 register limits.
 *
 * @param quanta Total quanta per bit (8..25)
 ...
 */
static bool CAN_SplitBit(uint32_t quanta, uint32_t *tseg1, uint32_t *tseg2,
                         uint32_t *sample_permille) {
    /* Quanta up to and including the sample point, i.e. sync + BS1. */
    uint32_t sampled =
        ((quanta * CAN_SAMPLE_POINT_PERMILLE) + CAN_PERMILLE_ROUND) / CAN_PERMILLE_FULL;
    uint32_t seg1 = 0;
    uint32_t seg2 = 0;
    ...
    if (sampled > (quanta - 1U)) {
        sampled = quanta - 1U;
    }

    seg1 = sampled - 1U;
    if (seg1 > CAN_TSEG1_MAX) {
        seg1 = CAN_TSEG1_MAX;
    }

    seg2 = quanta - 1U - seg1;
    if (seg2 < 1U || seg2 > CAN_TSEG2_MAX) {
        return false;
    }

    *tseg1 = seg1;
    *tseg2 = seg2;
    *sample_permille = ((1U + seg1) * CAN_PERMILLE_FULL) / quanta;
