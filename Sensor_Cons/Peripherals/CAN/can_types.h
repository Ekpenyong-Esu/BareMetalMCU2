...
#define CAN_MAX_FILTER_BANKS                                                                       \
    14U                      /*!< Filter banks on STM32F429 (single CAN); dual-CAN parts share 28 */
...
#define CAN_OP_MODE_SILENT_LOOPBACK                                                                \
    CAN_MODE_SILENT_LOOPBACK /*!< Silent + loopback for self-test                                  \
                              */
...
#define CAN_FILTER_MODE_ID_MASK                                                                    \
    CAN_FILTERMODE_IDMASK /*!< Mask mode: (id & mask) == (filter & mask) */
#define CAN_FILTER_MODE_ID_LIST                                                                    \
    CAN_FILTERMODE_IDLIST /*!< List mode: id equals one of two stored IDs */
...
typedef void (*CAN_TxCallback)(CAN_Handle_t *hcan,
                               uint8_t mailbox); /*!< Transmit complete on mailbox */
typedef void (*CAN_RxCallback)(CAN_Handle_t *hcan, const CAN_Frame *frame); /*!< Frame received */
typedef void (*CAN_ErrorCallbackFn)(CAN_Handle_t *hcan,
                                    CAN_ErrorType error); /*!< Bus error detected */
