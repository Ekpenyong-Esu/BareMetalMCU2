 ...
 * @param ident Identifier (11-bit standard or 29-bit extended)
 * @param frame_type Standard vs extended
 * @retval uint32_t Packed filter word
 */
static uint32_t CAN_PackFilter32(uint32_t ident, CAN_FrameType frame_type) {
    if (frame_type == CAN_FRAME_EXTENDED) {
        return (ident << CAN_FILTER32_EXID_SHIFT) | CAN_FILTER32_IDE_BIT;
    }

    return ident << CAN_FILTER32_STID_SHIFT;
}
 ...
 */
static uint16_t CAN_PackFilter16(uint32_t ident, CAN_FrameType frame_type) {
    if (frame_type == CAN_FRAME_EXTENDED) {
        return (uint16_t)(((ident >> CAN_FILTER16_EXID_SHIFT) & CAN_FILTER16_EXID_MASK) |
                          CAN_FILTER16_IDE_BIT);
    }

    return (uint16_t)((ident & CAN_STD_ID_MAX) << CAN_FILTER16_STID_SHIFT);
}
...
HAL_StatusTypeDef CAN_ConfigFilter(CAN_Handle_t *hcan, const CAN_FilterConfig *filter_config) {
    ...
    if (filter_config->scale == CAN_FILTER_SCALE_32BIT) {
        uint32_t packedId = CAN_PackFilter32(filter_config->id, filter_config->frame_type);
        uint32_t mask = CAN_PackFilter32(filter_config->mask, filter_config->frame_type);

        filter.FilterIdHigh = (uint16_t)(packedId >> CAN_ID_HIGH_SHIFT);
        filter.FilterIdLow = (uint16_t)(packedId & CAN_ID_LOW_MASK);
        ...
        uint16_t packedId = CAN_PackFilter16(filter_config->id, filter_config->frame_type);
        uint16_t mask = CAN_PackFilter16(filter_config->mask, filter_config->frame_type);

        filter.FilterIdHigh = packedId;
        filter.FilterIdLow = packedId;
