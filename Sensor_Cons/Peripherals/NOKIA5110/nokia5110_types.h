 ...
 * @details The application opens the SPI bus and names the three control
 *          lines; the driver owns nothing board-specific. RST may be left
 *          unwired (NULL port) when the module resets itself at power-on.
 */
typedef struct {
    SPI_Bus_t *Bus;        /*!< Bus the panel is wired to, already opened */
    GPIO_TypeDef *RstPort; /*!< Reset port, NULL if RST is not wired */
    uint16_t RstPin;
    GPIO_TypeDef *CePort; /*!< Chip-enable (SPI CS) port */
    uint16_t CePin;
