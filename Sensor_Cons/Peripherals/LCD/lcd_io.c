...
void LCD_IO_DelayUs(uint32_t us)
{
    const uint32_t cyclesPerUs = SystemCoreClock / 1000000U;
    volatile uint32_t count = (us * cyclesPerUs) / LCD_DELAY_LOOP_CYCLES;
...
void LCD_IO_DelayMs(uint32_t ms)
{
    HAL_Delay(ms);
}
...
void LCD_IO_WriteNibble(LCD_HandleTypeDef* handle, uint8_t nibble)
{
    ...
    LCD_IO_WriteDataPin(&pins->d4, nibble, 0x01U);
    LCD_IO_WriteDataPin(&pins->d5, nibble, 0x02U);
    LCD_IO_WriteDataPin(&pins->d6, nibble, 0x04U);
    LCD_IO_WriteDataPin(&pins->d7, nibble, 0x08U);

    LCD_IO_PulseEnable(handle);
}

void LCD_IO_WriteByte(LCD_HandleTypeDef* handle, uint8_t byte, uint8_t rs)
{
    const LCD_PinsTypeDef* pins = &handle->config.pins;

    LCD_IO_SetPin(&pins->rs, (rs != 0U) ? GPIO_PIN_SET : GPIO_PIN_RESET);
    ...
    if (handle->config.mode == LCD_MODE_8BIT)
    {
        LCD_IO_WriteDataPin(&pins->d0, byte, 0x01U);
        LCD_IO_WriteDataPin(&pins->d1, byte, 0x02U);
        LCD_IO_WriteDataPin(&pins->d2, byte, 0x04U);
        LCD_IO_WriteDataPin(&pins->d3, byte, 0x08U);
        LCD_IO_WriteDataPin(&pins->d4, byte, 0x10U);
        LCD_IO_WriteDataPin(&pins->d5, byte, 0x20U);
        LCD_IO_WriteDataPin(&pins->d6, byte, 0x40U);
        LCD_IO_WriteDataPin(&pins->d7, byte, 0x80U);

        LCD_IO_PulseEnable(handle);
    }
    else
    {
        LCD_IO_WriteNibble(handle, (uint8_t)((byte >> 4) & 0x0FU));
        LCD_IO_WriteNibble(handle, (uint8_t)(byte & 0x0FU));
    }
}
...
static void LCD_IO_PulseEnable(LCD_HandleTypeDef* handle)
{
    const LCD_PinTypeDef* en = &handle->config.pins.en;

    LCD_IO_SetPin(en, GPIO_PIN_RESET);
    LCD_IO_DelayUs(LCD_ENABLE_PULSE_US);
    LCD_IO_SetPin(en, GPIO_PIN_SET);
    LCD_IO_DelayUs(LCD_ENABLE_PULSE_US);
    LCD_IO_SetPin(en, GPIO_PIN_RESET);
