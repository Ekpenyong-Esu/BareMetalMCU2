...
static LCD_StatusTypeDef LCD_ResolveGeometry(const LCD_ConfigTypeDef* config,
...
static void LCD_SendResetSequence(LCD_HandleTypeDef* handle)
{
    if (handle->config.mode == LCD_MODE_4BIT)
    {
        ...
        LCD_IO_WriteByte(handle, LCD_CMD_FUNCTION_SET | LCD_4BIT_MODE | LCD_2LINE | LCD_5x8_DOTS, 0);
    }
    else
    {
        const uint8_t functionSet =
            LCD_CMD_FUNCTION_SET | LCD_8BIT_MODE | LCD_2LINE | LCD_5x8_DOTS;
