                                            ...
                                            OV7670_ResolutionTypeDef resolution)
{
    ...
    if (status != OV7670_OK || s_resolutions[resolution].dcwctrOverride == OV7670_DCWCTR_KEEP_TABLE_VALUE) {
        return status;
    }
...
OV7670_StatusTypeDef OV7670_ApplyFormat(OV7670_Handle_t *hov7670, OV7670_FormatTypeDef format)
{
    ...
    status = OV7670_UpdateReg(hov7670, OV7670_REG_COM7,
                              OV7670_COM7_FMT_MASK, s_formats[format].com7);
    if (status != OV7670_OK) {
        return status;
    }
