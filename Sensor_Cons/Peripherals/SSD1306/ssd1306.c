...
static const uint8_t SSD1306_InitSequence[] = {
    ...
    SSD1306_CMD_SET_DISPLAY_CLOCK_DIV, SSD1306_CLOCK_DIV_DEFAULT,
    SSD1306_CMD_SET_MULTIPLEX,         SSD1306_MULTIPLEX_64,
    SSD1306_CMD_SET_DISPLAY_OFFSET,    SSD1306_DISPLAY_OFFSET_NONE,
    ...
    SSD1306_CMD_SET_COM_PINS,          SSD1306_COM_PINS_ALTERNATIVE,
    SSD1306_CMD_SET_CONTRAST,          SSD1306_CONTRAST_DEFAULT,
    SSD1306_CMD_SET_PRECHARGE,         SSD1306_PRECHARGE_DEFAULT,
    SSD1306_CMD_SET_VCOM_DETECT,       SSD1306_VCOM_LEVEL_0_77,
...
SSD1306_StatusTypeDef SSD1306_Init(SSD1306_Handle_t *hssd, uint8_t address)
{
    ...
    status = SSD1306_IO_Init(&hssd->config);
    if (status != SSD1306_OK) {
        return status;
    }

    status = SSD1306_IO_WriteCommands(&hssd->config,
                                      SSD1306_InitSequence, sizeof(SSD1306_InitSequence));
    if (status != SSD1306_OK) {
        return status;
    }
    ...
    status = SSD1306_IO_WriteCommand(&hssd->config, SSD1306_CMD_DISPLAY_ON);
    if (status != SSD1306_OK) {
        hssd->initialized = false;
        return status;
    }
...
SSD1306_StatusTypeDef SSD1306_DeInit(SSD1306_Handle_t *hssd)
{
    ...
    status = SSD1306_IO_WriteCommand(&hssd->config, SSD1306_CMD_DISPLAY_OFF);
    hssd->initialized = false;
...
SSD1306_StatusTypeDef SSD1306_DisplayOn(SSD1306_Handle_t *hssd, bool on)
{
    ...
    return SSD1306_IO_WriteCommand(&hssd->config,
...
SSD1306_StatusTypeDef SSD1306_SetContrast(SSD1306_Handle_t *hssd, uint8_t contrast)
{
    ...
    return SSD1306_IO_WriteCommands(&hssd->config, frame, sizeof(frame));
}
...
SSD1306_StatusTypeDef SSD1306_InvertDisplay(SSD1306_Handle_t *hssd, bool invert)
{
    ...
    return SSD1306_IO_WriteCommand(&hssd->config,
                                   invert ? SSD1306_CMD_INVERT_DISPLAY
                                          : SSD1306_CMD_NORMAL_DISPLAY);
}
