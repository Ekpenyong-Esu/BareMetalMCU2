 ...
 * @note  Carries the frame buffer and the I2C staging buffer so that two
 *        panels on the same (or different) buses do not share state.
 */
typedef struct {
    SSD1306_Config_t config;                   /**< Display configuration */
    I2C_Device_t device;                       /**< This panel's record on the caller's bus */
    uint8_t buffer[SSD1306_BUFFER_SIZE];       /**< Frame buffer, one bit per pixel */
    uint8_t txBuffer[SSD1306_BUFFER_SIZE + 1]; /**< Control byte + payload, sent as one transfer */
