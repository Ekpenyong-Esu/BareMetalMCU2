...
float MIC_CalculatePeak(const int16_t *buffer, uint32_t length) {
    ...
    for (uint32_t i = 0; i < length; i++) {
        /* Widened: -INT16_MIN does not fit in an int16_t */
        int32_t abs_sample = (buffer[i] >= 0) ? buffer[i] : -(int32_t)buffer[i];
        if (abs_sample > peak) {
            peak = abs_sample;
        }
    }
...
bool MIC_IsNoiseGateOpen(const MIC_HandleTypeDef *hmic, float level) {
    ...
    float threshold_linear =
        powf(MIC_LOG_BASE, hmic->Config.NoiseGateThreshold / MIC_DB_SCALE_FACTOR);
...
void MIC_UpdateStatistics(MIC_HandleTypeDef *hmic, const int16_t *buffer, uint32_t length) {
    ...
    hmic->Statistics.AverageLevel =
        (1.0f - MIC_STATISTICS_ALPHA) * hmic->Statistics.AverageLevel + MIC_STATISTICS_ALPHA * rms;
