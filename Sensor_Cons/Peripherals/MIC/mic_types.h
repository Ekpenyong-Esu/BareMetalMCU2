...
/**
 * @brief Wiring and audio settings for one microphone.
 * @note  The wiring fields are read once by MIC_Init(); MIC_Configure() only
 *        applies the audio fields, so a handle never changes pins mid-life.
 */
typedef struct {
    /* Wiring, chosen by the application */
    SPI_TypeDef *i2sInstance; /**< SPI2 (I2S2) or SPI3 (I2S3) */
    GPIO_TypeDef *clkPort;    /**< I2Sx_CK, the PDM clock driven to the mic */
    uint16_t clkPin;
    GPIO_TypeDef *dataPort; /**< I2Sx_SD, the PDM bitstream from the mic */
    uint16_t dataPin;
    uint8_t alternate;             /**< GPIO_AFx_SPIx, or 0 to derive it from i2sInstance */
    DMA_Stream_TypeDef *dmaStream; /**< Stream that serves SPIx_RX */
    uint32_t dmaChannel;           /**< DMA_CHANNEL_x selecting SPIx_RX on that stream */

    /* Audio, may be changed later with MIC_Configure() */
