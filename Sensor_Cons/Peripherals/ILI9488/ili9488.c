 ...
 */
static ILI9488_StatusTypeDef ILI9488_RunInitSequence(const ILI9488_Config_t *config)
{
    for (uint32_t i = 0U; i < (sizeof(ILI9488_InitSequence) / sizeof(ILI9488_InitSequence[0])); i++) {
        const ILI9488_InitStep_t *step = &ILI9488_InitSequence[i];
        ILI9488_StatusTypeDef status = ILI9488_IO_WriteCommand(config, step->command);
        ...
        if (step->paramCount > 0U) {
            status = ILI9488_IO_WriteData(config, step->params, step->paramCount);
            if (status != ILI9488_OK) {
                return status;
            }
        }
                                                         ...
                                                         uint16_t *height)
{
    switch (orientation) {
        ...
        case ILI9488_ORIENTATION_LANDSCAPE_REV:
            *madctl = ILI9488_MADCTL_MY | ILI9488_MADCTL_MX | ILI9488_MADCTL_MV | ILI9488_MADCTL_BGR;
            *width = ILI9488_HEIGHT;
            *height = ILI9488_WIDTH;
            break;
        default:
                                   ...
                                   GPIO_TypeDef *rst_port, uint16_t rst_pin)
{
    ...
    status = ILI9488_IO_Init(&hili->config);
    if (status != ILI9488_OK) {
        return status;
    }

    status = ILI9488_RunInitSequence(&hili->config);
    if (status != ILI9488_OK) {
...
ILI9488_StatusTypeDef ILI9488_SetOrientation(ILI9488_Handle_t *hili, ILI9488_Orientation_t orientation)
{
    ...
    status = ILI9488_IO_WriteCommand(&hili->config, ILI9488_CMD_MEMORY_ACCESS_CTL);
    if (status != ILI9488_OK) {
        return status;
    }

    status = ILI9488_IO_WriteData(&hili->config, &madctl, 1U);
    if (status != ILI9488_OK) {
        return status;
    }
...
ILI9488_StatusTypeDef ILI9488_Clear(ILI9488_Handle_t *hili, uint16_t color)
{
    ...
    status = ILI9488_IO_SetAddressWindow(&hili->config, 0U, 0U,
                                         (uint16_t)(hili->width - 1U),
    ...
    return ILI9488_IO_WritePixels(&hili->config, color,
                                  (uint32_t)hili->width * (uint32_t)hili->height);
}
