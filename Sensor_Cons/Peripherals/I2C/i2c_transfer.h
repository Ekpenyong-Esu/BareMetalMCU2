 ...
 * @details Every transfer names the device it targets. The device carries the
 *          bus it sits on, its slave address and its bus settings, so
 *          selecting the bus and addressing the slave stay in step.
 ******************************************************************************
 */
...
/**
 * @brief Probe every address on a device's bus
 * @param device Supplies the bus and the settings to scan with; its own
 *               address is ignored because the scan walks the whole range.
 */
