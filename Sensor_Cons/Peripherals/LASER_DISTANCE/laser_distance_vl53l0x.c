...
LASER_DISTANCE_StatusTypeDef LASER_DISTANCE_VL53L0X_Init(LASER_DISTANCE_Handle_t *hlaser) {
    ...
    if (LASER_DISTANCE_I2C_ReadReg(hlaser, VL53L0X_REG_IDENTIFICATION_MODEL_ID, &deviceId) !=
        LASER_DISTANCE_OK) {
    ...
    if (deviceId != VL53L0X_EXPECTED_DEVICE_ID) {
        log_error("LASER_DISTANCE: Invalid VL53L0X device ID: 0x%02X, expected: 0x%02X", deviceId,
                  VL53L0X_EXPECTED_DEVICE_ID);
...
LASER_DISTANCE_StatusTypeDef LASER_DISTANCE_VL53L0X_ReadRange(LASER_DISTANCE_Handle_t *hlaser) {
    ...
    if (LASER_DISTANCE_I2C_ReadMulti(hlaser, VL53L0X_REG_RESULT_RANGE_STATUS, rangeData,
                                     VL53L0X_RANGE_DATA_SIZE) != LASER_DISTANCE_OK) {
        return LASER_DISTANCE_I2C_ERROR;
    }
...
bool LASER_DISTANCE_VL53L0X_IsReady(LASER_DISTANCE_Handle_t *hlaser) {
    ...
    if (LASER_DISTANCE_I2C_ReadReg(hlaser, VL53L0X_REG_RESULT_INTERRUPT_STATUS, &status) !=
        LASER_DISTANCE_OK) {
        return false;
    }
...
LASER_DISTANCE_StatusTypeDef LASER_DISTANCE_VL53L0X_SetAddress(LASER_DISTANCE_Handle_t *hlaser,
    ...
    if (status == LASER_DISTANCE_OK) {
        /* The chip now answers at the new address, so the bus record must too. */
        hlaser->config.i2cAddress = newAddress;
        hlaser->device.address = newAddress;
    }
