...
/* Private defines -----------------------------------------------------------*/
#define SYS_MICROS_PER_SECOND 1000000U /**< DWT cycles per second -> per microsecond */

/* PLL factors: 8 MHz HSE / M * N / P = 168 MHz SYSCLK, / Q = 48 MHz USB clock */
#define SYS_PLL_M 4U
#define SYS_PLL_N 168U
#define SYS_PLL_Q 7U
...
static void SYS_MicrosInit(void) {
    ...
    s_cyclesPerMicrosecond = SystemCoreClock / SYS_MICROS_PER_SECOND;
...
void SystemClock_Config(void) {
    ...
    RCC_OscInitStruct.PLL.PLLM = SYS_PLL_M;     /* 8 MHz HSE / 4 = 2 MHz PLL input */
    RCC_OscInitStruct.PLL.PLLN = SYS_PLL_N;     /* 2 MHz * 168 = 336 MHz VCO */
    RCC_OscInitStruct.PLL.PLLP = RCC_PLLP_DIV2; /* 336 MHz / 2 = 168 MHz SYSCLK */
    RCC_OscInitStruct.PLL.PLLQ = SYS_PLL_Q;     /* 336 MHz / 7 = 48 MHz for USB OTG FS */
