...
#define SSD1331_RGB(r, g, b)                                                                       \
    ((SSD1331_Color_t)((((uint16_t)(r)&0xF8U) << 8) | (((uint16_t)(g)&0xFCU) << 3) |               \
                       ((uint16_t)(b) >> 3)))
 ...
 * @details The application opens the SPI bus and hands it over; the panel
 *          only adds three private control lines. Reset may be left out by
 *          setting @p rstPort to NULL, in which case the module's RES pin
 *          must be tied high on the board.
 */
typedef struct {
    SPI_Bus_t *bus;             /**< Bus the panel is wired to, already opened */
    uint32_t baudRatePrescaler; /**< SPI_BAUDRATEPRESCALER_x, or 0 for the driver's default */
...
typedef struct {
    ...
    SPI_Device_t device;                      /**< This panel's slot on the shared bus */
    uint16_t currentX;                        /**< Text cursor column in pixels */
    uint16_t currentY;                        /**< Text cursor row in pixels */
    bool initialized;                         /**< Initialization status */
    uint8_t frameBuffer[SSD1331_BUFFER_SIZE]; /**< Pixels in the panel's byte order */
} SSD1331_Handle_t;
