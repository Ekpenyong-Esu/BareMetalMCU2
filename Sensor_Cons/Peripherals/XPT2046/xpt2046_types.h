 ...
 * @note  The bus is opened by the application; the driver only owns the chip
 *        select and PENIRQ pins and its slot on that bus.
 */
typedef struct {
    SPI_Bus_t *bus;         /**< Bus the controller is wired to, already open */
 ...
 */
typedef struct {
    SPI_Device_t device;        /**< This controller's slot on the bus */
