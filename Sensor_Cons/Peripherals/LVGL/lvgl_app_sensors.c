...
static const int32_t s_chart_seed[SENSOR_CHART_POINTS] = {
    10, 20, 35, 45, 50, 55, 60, 70, 65, 50
};
...
static void LVGL_Sensors_CreateChart(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(s_chart, 200, 120);
    ...
    s_chart_series = lv_chart_add_series(s_chart, lv_color_hex(UI_COLOR_ACCENT),
...
static void LVGL_Sensors_CreateCards(lv_obj_t *parent)
{
    for (int i = 0; i < SENSOR_CARD_COUNT; i++) {
        ...
        lv_obj_align(card, LV_ALIGN_BOTTOM_LEFT,
                     SENSOR_CARD_ORIGIN_X + (i * SENSOR_CARD_PITCH), -50);
