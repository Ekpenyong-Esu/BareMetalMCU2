...
static void LVGL_Nav_EventHandler(lv_event_t *e)
{
    uintptr_t id;
    lv_obj_t *target;
    ...
    if (id >= (uintptr_t)LVGL_SCREEN_COUNT) {
        return;
    }

    target = LVGL_Screens_Get((LVGL_ScreenId_t)id);
    if (target == NULL) {
        return;
    }
...
void LVGL_Nav_Attach(lv_obj_t *btn, LVGL_ScreenId_t target)
{
    ...
    lv_obj_add_event_cb(btn, LVGL_Nav_EventHandler, LV_EVENT_CLICKED,
