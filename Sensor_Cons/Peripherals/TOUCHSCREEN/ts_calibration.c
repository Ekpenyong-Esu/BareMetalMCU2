 ...
 */
static int32_t TS_MapRange(int32_t value, int32_t inMin, int32_t inMax, int32_t outMin,
                           int32_t outMax) {
    if (inMax == inMin) {
        return outMin;
    }
...
TS_StatusTypeDef TS_MapToDisplay(const TS_HandleTypeDef *hts, uint16_t rawX, uint16_t rawY,
    ...
    const int32_t width = (int32_t)hts->Config.displayWidth;
    const int32_t height = (int32_t)hts->Config.displayHeight;

    int32_t mappedX = TS_MapRange((int32_t)rawX, (int32_t)hts->Calibration.MinX,
                                  (int32_t)hts->Calibration.MaxX, width - 1, 0);

    int32_t mappedY = TS_MapRange((int32_t)rawY, (int32_t)hts->Calibration.MinY,
                                  (int32_t)hts->Calibration.MaxY, height - 1, 0);

    *dispX = TS_ClampToDisplay(mappedX, width);
    *dispY = TS_ClampToDisplay(mappedY, height);
