...
HAL_StatusTypeDef LTDC_ClearFramebuffer(LTDC_Driver_t *driver, uint8_t layer, uint32_t color)
{
    ...
    LTDC_FillPixels((uint8_t *)(uintptr_t)cfg->framebufferAddress,
                    (uint32_t)cfg->imageWidth * cfg->imageHeight,
                    color,
                    cfg->pixelFormat);
...
HAL_StatusTypeDef LTDC_CopyFramebuffer(LTDC_Driver_t *driver, uint8_t srcLayer, uint8_t dstLayer)
{
    ...
    memcpy((void *)(uintptr_t)dst->framebufferAddress,
           (void *)(uintptr_t)src->framebufferAddress,
...
HAL_StatusTypeDef LTDC_DrawPixel(LTDC_Driver_t *driver, uint16_t x, uint16_t y, uint32_t color)
{
    ...
    if (x < cfg->windowX0 || x > cfg->windowX1 || y < cfg->windowY0 || y > cfg->windowY1) {
        driver->errorCode = LTDC_ERROR_INVALID_PARAM;
        return HAL_ERROR;
    }

    uint32_t index = ((uint32_t)(y - cfg->windowY0) * cfg->imageWidth) + (uint32_t)(x - cfg->windowX0);
