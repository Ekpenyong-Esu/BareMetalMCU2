...
#define LCD_BL_GPIO_Port GUI_BOARD_LCD_BL_PORT
#define LCD_BL_Pin GUI_BOARD_LCD_BL_PIN
...
static void APP_DimAnimReady_PowerOff(lv_anim_t *anim) {
    ...
    if (!ILI9341_SleepIn(GUI_Board_Panel())) {
        log_error("APP: panel did not acknowledge sleep; powering down anyway");
    }

    if (GUI_Board_Ltdc()->Instance != NULL) {
        __HAL_LTDC_DISABLE(GUI_Board_Ltdc());
    }
