...
#define GUI_INFO_TITLE_Y 10    /* Title offset from the top edge */
#define GUI_INFO_CARD_SIZE 200 /* Square info card side */

static const char *const s_info_text = "MCU: STM32F429ZI\n"
                                       "Core: ARM Cortex-M4\n"
                                       "Freq: 168 MHz\n"
                                       "Flash: 2 MB\n"
                                       "RAM: 256 KB\n"
...
void LVGL_Info_Create(void) {
    ...
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, GUI_INFO_TITLE_Y);

    lv_obj_t *card = lv_obj_create(screen);
    lv_obj_set_size(card, GUI_INFO_CARD_SIZE, GUI_INFO_CARD_SIZE);
    lv_obj_center(card);
