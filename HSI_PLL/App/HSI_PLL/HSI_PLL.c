...
/**
 * @brief Initialize HSI with PLL to reach 180MHz system clock
 *
 * This function:
 * 1. Enables HSI (16MHz internal oscillator)
 * 2. Configures PLL to use HSI as input
...
void HSI_PLL_Init(void) {
    ...
    // 4. Configure PLL
    // HSI = 16MHz, /M=16, *N=360, /P=2 → 180MHz
    RCC_PLLCFGR = RCC_PLLCFGR_PLLSRC_HSI | // HSI as source
                  (16 << 0) |               // PLLM = 16
                  (360 << 6) |              // PLLN = 360
                  (0 << 16) |               // PLLP = 2 (00 = 2)
